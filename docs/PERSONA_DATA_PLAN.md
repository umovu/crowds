...
(Nkosinathi, 2026-07-04; full voice note digested in §6a): the library reads "NGO-poor" — every voice is a hardship story,
...
## 6. Tester feedback #2 — Tumelo (2026-07-07, agri/rural)

Second tester, tried the tool on a rural/agri product with grant-user personas.
Four points; they largely validate what's already planned above.

### His observations
1. **Two farmer archetypes the library misses.** (a) *Communal farmer*: owns
   1–10 cattle plus goats/sheep; income = grants + occasional livestock sales;
   psychology = community norms, rural safety, "does this improve my
   community". (b) *Emerging farmer*: white-collar qualification, lives in the
   city, owns a farm emakhaya, manages remotely by sending money; cares about
   asset protection (stock theft) and livestock productivity. His grant-user
   test failed on salience because "grant user" is a payment channel, not an
   identity — a grant recipient *with livestock* cares about stock theft; one
   without doesn't. Live instance of the assets gap (§1) and the money chorus.
2. **Industry-first entry.** Run sims by vertical + product economics (average
   ticket size, sales cadence); backend recruits the relevant personas from
   that — high-ticket/frequent → higher-LSM cast.
3. **Academic grounding.** Ground sims in sociological papers ("aggregated
   surveys with the insights behind them"); hover over output → paper refs.
   He used papers himself when entering new markets.
4. **Positioning:** "market research co-pilot" — second independent tester to
   converge on the panel-wedge framing.

### Disposition
- **Farmer archetypes → build pipeline.** Add `communal_livestock_farmer` and
  `remote_emerging_farmer` archetypes from QLFS agriculture bodies + GHS asset
  flags (livestock ownership where available). The remote-manager pattern needs
  an urban-income/rural-asset link — note as a candidate field in step 1
  enrichment. Slots into steps 1–2; no new architecture.
- **Ticket size/cadence as casting signal → step 6.** Extends the planned
  product-mode tier tilt in `select_for_query`: price point weights the
  tier/LSM mix deterministically (real income vs price, LLM-off testable).
  **Weight, don't filter** — "this segment can't afford you and why" is part of
  the product's value; never silently drop low-tier personas.
- **Academic grounding → designed, phased.** Full design + starter corpus in
  `docs/ACADEMIC_GROUNDING.md`: pre-selected papers distilled offline into
  human-reviewed mechanism cards (qualitative-only, segment-tagged), runtime
  binding as deterministic lookup alongside the economic lens, citations
  surfaced in UI; live OpenAlex retrieval later feeds a review queue only.
- **Positioning:** reflect market-research-co-pilot language in landing copy on
  next touch.

## 7. Measurement (keeps us honest)
