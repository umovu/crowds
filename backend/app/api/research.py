...
def list_personas():
    ...
    try:
        ...
        # carry the survey-grounded fee-band signal. The runtime cache
        # (uploads/persona_cache/) doesn't — it was generated by a path that
        # never knew about GHS fee data. Match cache entries against the
        # library on (name, archetype) and inherit `fees_band` /
        # `learner_fee_bands` so the Cast picker's "Fee-paying households"
        # and "No-fee-school households" groups light up. Read-only join —
        ...
        if os.path.exists(lib_path):
            try:
                ...
                logger.warning(f"Persona library load failed for fee enrichment: {e}")
